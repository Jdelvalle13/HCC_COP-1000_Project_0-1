...
def reset_authorized_vehicles_file (authorizedVehicles):
    ...
    while True:
        ...
        reset = input('Are you sure you wish to proceed?: ').strip().lower()

        # Restart confirmation loop in event of invalid input and provide message to user
        if reset.lower() not in ['yes', 'no']:
            print('Invalid response entered.')
            print('Please enter "No" to cancel and return to main menu.')
            print(f'Please enter "Yes" to "{menuOptions[4]}".')
            print()
            continue

        # If is confirmation is no, print confirmation message and return to main menu
        if reset.lower() == 'no':
            print(f"{menuOptions[4]} cancelled.")
            return
        
        # If user confirmation is yes, reset file to original "allowedVehiclesList"
        if reset == 'yes':
